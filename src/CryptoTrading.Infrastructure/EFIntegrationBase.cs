...
public class EFIntegrationBase
{
    ...
    public EFIntegrationBase(CryptoTradingDbContext dbContext)
    {
     ...
     public virtual async Task BulkInsert<T>(IEnumerable<T> data) where T : class
    {
        var bulkconfig = new BulkConfig { BatchSize = 50000, BulkCopyTimeout = 1500, SqlBulkCopyOptions = EFCore.BulkExtensions.SqlBulkCopyOptions.KeepIdentity };
